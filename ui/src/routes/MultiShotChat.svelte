<script lang="ts">
  ...
  function restoreChat(item: ListItem) {
    tokenHistory = [...item.tokenHistory];
    clearResult();
    tokenHistory.forEach((token) => {
      const type = token.role === 'user' ? 'user' : 'ai';
      const bubbleId = addBubble(token.role === 'user' ? 'User' : 'AI', type);
      printMessage(bubbleId, token.content);
    });
  }
    ...
    <div id="search" class="grid grid-rows-[1fr_auto] gap-0 hide-on-small">
      ...
      <div class="bg-surface-600/30 p-4">
        {#each listItems as item (item.id)}
          <button
            class="cursor-pointer hover:bg-surface-700/30 p-2 w-full text-left"
      ...
      <div class="bg-surface-600/30 p-4">
        <button class="btn variant-filled-primary" on:click={handleClearList}>Clear List</button>
      </div>
